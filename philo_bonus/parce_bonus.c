...
int init_semaphores(t_rules *rules)
{
    ...
    if (rules->print == SEM_FAILED)
        return (1);
    rules->death_sem = sem_open("/death_sem", O_CREAT, 0644, 0);
    if (rules->death_sem == SEM_FAILED)
        return (1);
    rules->finished_sem = sem_open("/finished_sem", O_CREAT, 0644, 0);
    if (rules->finished_sem == SEM_FAILED)
        return (1);
